...
class InvestmentImportTest < ActiveSupport::TestCase
  ...
  test "provides CSV template" do
    ...
    # Headers use * suffix to indicate required fields
    assert template.headers.any? { |h| h.to_s.include?("date") }
    assert template.headers.any? { |h| h.to_s.include?("transaction_type") }
  ...
  test "provides dry run summary" do
    ...
    assert summary.key?(:transactions)
  end
end
